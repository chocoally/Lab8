...
Beta takes each gene (up, down, DE) and estimates the binding site within a distance from the TSS. It then scores the gene's regulatory potential (binding sites further away have a lower score contribution to the overall score). It then ranks all of the genes by the score. It then calculates a cumulative distribution function and uses a K-S test to determine whether the groups (up, down) differn significantly from the unchanged gene group
...
Rank peaks based on regulatory potential - distance of peak from gene TSS
Rank genes based on differential expression
So for each peak (ranked) there is a associated rank for each associated diff gene expression. High rank peak with high rank associated DE would lead to lower p-value.
...
Can look at JASPER database for binding motifs for each TF
...
dinucleotide_dictionary.update((x, y/curr_total) for x, y in dinucleotide_dictionary.items()) #now percentage
...
seq_lengths = 8
seq_array = list()
dinucleotides = list(dinucleotide_dictionary.keys())
probs = list(dinucleotide_dictionary.values())
